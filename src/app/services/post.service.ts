...
export class PostService {
  ...
  public createOrUpdatePost(post: Post): Promise<void> {
    if (!post.id) {
      ...
      post.id = this.afStore.collection<Post>(PostPath).ref.doc().id;
      return this.afStore.collection<Post>(PostPath).doc(post.id).set(Object.assign({}, post));
    } else {
      // Post already exists, just update the content
      return this.afStore.collection<Post>(PostPath).ref.doc(post.id).update('content', post.content);
    }
  }
